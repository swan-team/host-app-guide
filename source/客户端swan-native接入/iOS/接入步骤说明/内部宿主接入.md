...
* [注] 小程序内部需要请求配置（后期废弃），需要composeParameters （appname、osname换成宿主自己的、uid需要使用cuid）、rootServerHost、currentNetworkTypeString方法，拷贝开源demo中提供的实现；按照[公参接入说明](公参接入.md)实现公参接口;
