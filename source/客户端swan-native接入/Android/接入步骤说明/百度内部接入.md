...
私有API主要用于和百度业务强相关的需求实现，比如百度APP首页下拉二楼、小程序广告、春节运营活动等

> 此列表基于客户端SDK - 2.5.0
>  
> 具体实现可参考[手百主线小程序仓库](http://icode.baidu.com/repos/baidu/baiduapp-android/aiapps/) 或 [小程序内部开源仓库](http://icode.baidu.com/repos/baidu/searchbox-android/aiapps/)（内部开源仓库仅实现部分私有API）  

| API描述| 参考实现类 |
| --- | --- |
| 获取Bduss | GetBdussAction |
| 获取SToken | GetStokenAction |
| 获取用户信息 | PrivateGetUserInfoAction |
| 获取用户实名信息 | GetRealNameInfoAction |
| 推荐相似商品 | RecommendSimilarAction |
| 推荐商品 | RecommendProductsAction |
| 获取小程序历史 | GetSwanHistoryAction |
| 删除小程序历史 | RMSwanHistoryAction |
| 调起聚合钱包支付 | RequestPolymerPaymentAction |
| 百度APP内部跳转 | PageTransitionAction |
| 百度APP消息通道 | SwanAppChannelAction |
| 百度APP拉取IM消息 | SwanAppPullMsgAction |
| AR相机-添加 | ARCameraInsertAction |
| AR相机-更新 | ARCameraUpdateAction |
| AR相机-拍照 | ARCameraTakePhotoAction |
| AR相机-开始录制 | ARCameraStartRecordAction |
| AR相机-结束录制 | ARCameraStopRecordAction |
| AR相机-移除 | ARCameraRemoveAction |
| 执行AI相关网络请求 | AigRequestAction |
| 获取账号相关通用信息（CUID、UID等） | GetSysInfoAction |
| 语音识别 | VoiceRecognitionAction |
| 打开广告落地页 | AdLandingAction |
| 广告组件专用网路请求 | AdRequestAction |
| 广告组件专用APP下载能力 | SwanAdAppDownloadAction |
| 获取用户Push权限状态 | GetPushSettingStateSyncAction |
| 引导用户开启Push权限 | GuidePushSettingAction |
| 调起第三方登录 | ThirdPartyLoginAction |
| 读取手机通讯录 | ReadContactsAction |
| 图像识别 | ImageRecognitionAction |
| 启动云游戏插件 | LaunchCloudGameAction |
| 小程序闪付 | QuickPayAction |
